...
export default function FavoritesPage() {
  ...
  if (!currentMember) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-160px)] font-sans gap-4">
        <span className="text-zinc-400 font-bold text-sm">กรุณาเข้าสู่ระบบเพื่อดูรายการโปรด</span>
        <Link
          href="/login"
  ...
  // Filter inventions that are favorited
  const favoriteInventions = inventions.filter((inv) => isFavorite(inv.inventionId));
    ...
    <div className="flex flex-col gap-5 font-sans">
      ...
      {favoriteInventions.length === 0 ? (
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800/80 rounded-[32px] p-16 flex flex-col items-center justify-center gap-4 shadow-xs text-center">
          <span className="text-zinc-300 text-6xl">❤️</span>
          <h3 className="font-extrabold text-zinc-650 mt-2 text-base">ยังไม่มีรายการโปรด</h3>
          <p className="text-xs text-zinc-400 font-bold max-w-xs leading-normal">
            คุณสามารถกดไอคอนหัวใจที่การ์ดสิ่งประดิษฐ์ต่างๆ ในหน้าหลักเพื่อบันทึกไว้เป็นรายการโปรดได้ที่นี่
          </p>
          <Link
            href="/"
                ...
                <div>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <button
                      ...
                      className="p-2 rounded-full hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-all cursor-pointer text-red-500 hover:text-red-650"
                    >
                      <svg
                        className="w-6 h-6"
                        fill="currentColor"
